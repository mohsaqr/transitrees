...
test_that("tune_pathtree returns a sorted pathtree_tune data.frame", {
  m <- mk_iid_data()
  tg <- tune_pathtree(m, max_depth = 1L:2L, nmin = c(3L, 5L),
                      smoothing = "floor",
                      prune = c(FALSE, TRUE), k = 3L)
...
test_that("tune_pathtree compares smoothing methods cleanly", {
  m <- mk_iid_data()
  tg <- tune_pathtree(m, max_depth = 1L:2L, nmin = 3L,
...
test_that("tune_pathtree on i.i.d. data prefers smallest max_depth", {
  ...
  tg <- tune_pathtree(m, max_depth = 1L:3L, nmin = 3L,
                      smoothing = "floor", prune = FALSE, k = 4L)
  best <- attr(tg, "best")
...
test_that("tune_pathtree print method runs without error", {
  m <- mk_iid_data()
  tg <- tune_pathtree(m, max_depth = 1L, nmin = 3L,
                      smoothing = "floor", prune = FALSE, k = 3L)
...
test_that("tune_pathtree is reproducible across calls with same seed", {
  m <- mk_iid_data()
  a <- tune_pathtree(m, max_depth = 1L:2L, nmin = 3L,
                     smoothing = "floor", prune = FALSE, k = 3L,
                     seed = 42L)
  b <- tune_pathtree(m, max_depth = 1L:2L, nmin = 3L,
                     smoothing = "floor", prune = FALSE, k = 3L,
                     seed = 42L)
...
test_that("tune_pathtree errors when k > n_sequences", {
  ...
  expect_error(tune_pathtree(m, k = 10L), "Not enough sequences")
})
