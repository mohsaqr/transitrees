...
test_that("tree_dependence returns a sorted data.frame with all columns", {
  tree <- .simple_tree()
...
test_that("base 2 vs base e differ by ln(2)", {
  tree <- .simple_tree()
  pd2 <- tree_dependence(tree, base = 2)
  pde <- tree_dependence(tree, base = exp(1))
...
test_that("tree_dependence on an order-1 generator has small chain-level KL", {
  tree <- .simple_tree()
  pr   <- prune_tree(tree, criterion = "G2", alpha = 0.05)
  pd   <- tree_dependence(pr)
...
test_that("tree_dependence sorts descending by each sort_by column", {
  tree <- .simple_tree()
  for (sb in c("divergence", "entropy_drop", "entropy", "count", "depth")) {
    pd  <- tree_dependence(tree, sort_by = sb)
    key <- pd[[sb]]
    expect_equal(key, sort(key, decreasing = TRUE), info = sb)
  }
})

test_that("tree_dependence top = keeps the leading rows of the sort", {
  tree <- .simple_tree()
  full <- tree_dependence(tree, sort_by = "entropy_drop")
  pd3  <- tree_dependence(tree, sort_by = "entropy_drop", top = 3L)
